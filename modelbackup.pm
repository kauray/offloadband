...
player scheduler
	[deviceex], [serverex], [retaindevice], [retainserver], [migtoserver], [migtodevice], [migdone], [migdonedev], [notaskmig], [devicerunningcost], [core1], [core2], [migonetwo], [migtwoone], [retaincore1], [retaincore2]
endplayer
...
formula etdt = batteryenergy - econsumed;
formula econsumed = slottime  * p_device * e_loss;
...
const double tenv;
const double rcpuenv = 78.8;
const double rbatenv = 7.58;
const double rcpubat = 0; //to be calculated
const double pbat;

formula tempbattery = tenv + (rcpuenv * rbatenv) / (rbatenv + rcpubat + rcpuenv) * pcpu  + 
((rcpuenv * rbatenv) + (rcpubat*rbatenv)) / (rbatenv + rcpubat + rcpuenv) * pbat;
...
module offloadchoice
    ...
    [retaindevice] turn = 0 & mod(dtime,k) = 0 & execution = 0 & tmig = 0 -> (tmig' = 0) & (execution' = 0) & (turn' = 1); // retain
...
module device
    ...
    [migonetwo] turn = 1 & mod(dtime,k) = 0 & dtime < tmax & core = 1 & execution = 0 -> (core' = 2) & (mig' = 1) & (turn' = 1); // migrate
    [migtwoone] turn = 1 & mod(dtime,k) = 0 & dtime < tmax & core = 2 & execution = 0 -> (core' = 1) & (mig' = 2) & (turn' = 1); // migrate
    ...
    [devicerunningcost] turn = 2 & execution = 0 & dtime < tmax & batteryenergy > 0 & mig = 0 -> 
  (batteryenergy' = floor(etdt)) & (dtime' = dtime + 1) & (mig' = 0) & (turn' = 0);
    [devicerunningcost] turn = 2 & execution = 0 & dtime < tmax & batteryenergy > 0 & mig = 1  -> 
  (batteryenergy' = floor(etdt)) & (dtime' = dtime + 1) & (mig' = 0) & (turn' = 0);
  ...
  (batteryenergy' = floor(etdt)) & (dtime' = dtime + 1) & (mig' = 0) & (turn' = 0);
	...
	(batteryenergy' = floor(etdt)) & (turn' = 0) & (dtime' = dtime + 1);
